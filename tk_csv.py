...
#입력상자 설명 레이블
label = Label(window, text="시설별 기구 종류(목록보고 이름입력!) ") #Label은 라벨링 텍스트를 넣을수 있는공간
label.grid(row=0, column=0, sticky=W) #W는 west다 고정시킬 위치 grid는 위치에 장착??


values=[str(i)+"번 "+ data.loc[i][0] for i in range(56) ]
combobox = ttk.Combobox(window, height=15, values=values)
